...
-- Added for Orleans runtimes that require a defunct-entry cleanup query which the
-- shipped PostgreSQL setup script does not define. Deletes non-active membership rows
-- for the deployment that have not reported alive since the given time.
INSERT INTO OrleansQuery(QueryKey, QueryText)
VALUES
(
